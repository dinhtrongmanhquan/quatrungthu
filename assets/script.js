...
const rabbitLetters = [
  {
    ...
    letterColor: "#ff5c8a",
    title: "Bức Thư Hồng 💗",
    textFile: "./assets/3.txt",
  },
  {
    color: "#ffd43b",
    letterColor: "#ffd43b",
    title: "Bức Thư Vàng 💛",
    textFile: "./assets/2.txt",
  },
  {
    color: "#5ac8fa",
    letterColor: "#5ac8fa",
    title: "Bức Thư Xanh 💙",
    textFile: "./assets/1.txt",
  },
];

function createRabbit(rabbitData) {
  const group = new THREE.Group();
  const rabbitMat = new THREE.MeshStandardMaterial({
    color: rabbitData.color,
    emissive: rabbitData.color,
  ...
  // Vùng vô hình để bấm vào thỏ dễ hơn
  ...
  hitMesh.position.set(0, 0.65, 0.15);
  hitMesh.userData.rabbit = group;
  group.add(hitMesh);
...
const rabbits = [];
for (let i = 0; i < 3; i++) {
  const rabbitData = rabbitLetters[i];
  const { group: rabbitMesh, hitMesh } = createRabbit(rabbitData);
...
// Cho phép bấm trực tiếp vào 3 chú thỏ để mở thư riêng
rabbits.forEach((rabbit) => {
  interactiveObjects.push(rabbit.hitMesh);
});
...
// Mỗi lời chúc sẽ có một màu đèn riêng theo thứ tự.
// Nếu thêm nhiều lời chúc hơn số màu, màu sẽ tự lặp lại từ đầu.
const lanternColors = [
  "#ff3b30", // đỏ
  "#ffd60a", // vàng
  "#34c759", // xanh lá
...
for (let i = 0; i < lanternCount; i++) {
  ...
  // Chọn lời chúc ngẫu nhiên và lấy màu tương ứng với lời chúc đó.
  const wishIndex = Math.floor(Math.random() * wishList.length);
  const wishData = wishList[wishIndex];
  const lanternColor = lanternColors[wishIndex % lanternColors.length];

  // Đổi màu thân đèn theo lời chúc.
  const body = lantern.children[0];
  if (body && body.material) {
    body.material.color.set(lanternColor);
    body.material.emissive.set(lanternColor);
    body.material.emissiveIntensity = 0.7;
  }

  // Đổi màu bảng tên và ánh sáng xung quanh đèn.
  const tag = lantern.children[2];
  if (tag && tag.material) {
    tag.material.color.set(lanternColor);
  }

  const glow = lantern.children[3];
  if (glow && glow.material) {
    glow.material.color.set(lanternColor);
  }
...
function typeWriter(element, text, speed = 32) {
  ...
  function writeNext() {
    ...
    index++;
    typewriterTimer = setTimeout(writeNext, speed);
  }
...
function onPointerUp(event) {
  ...
  if (intersects.length > 0) {
    ...
    // Bấm vào thỏ -> mở bức thư tương ứng
    if (hitMesh.userData.rabbit) {
      const rabbit = rabbits.find((r) => r.mesh === hitMesh.userData.rabbit);
      if (rabbit) {
        selectedLantern = null;
        fetch(new URL(rabbit.letter.textFile, window.location.href))
          .then(response => {
            if (!response.ok) {
          ...
          .then(text => {
            typeWriter(wishText, text, 32);
          })
          .catch(error => {
            console.error(error);
            wishText.textContent =
              "Không thể mở bức thư. Hãy kiểm tra file " +
              rabbit.letter.textFile;
          });
        wishImage.removeAttribute("src");
        wishImage.style.display = "none";

        const container = wishImage.parentElement;
        container.style.display = "none";

        wishTitle.textContent = rabbit.letter.title;
        wishTitle.style.color = rabbit.letter.letterColor;
        wishModal.querySelector(".tag").textContent = "✦ Một Bức Thư Gửi Riêng Cậu ✦";
        wishModal.querySelector(".tag").style.color = rabbit.letter.letterColor;

        const card = wishModal.querySelector(".wish-card");
        card.style.borderColor = rabbit.letter.letterColor;
        card.style.boxShadow = `0 0 35px ${rabbit.letter.letterColor}55, inset 0 0 15px ${rabbit.letter.letterColor}22`;
    ...
    // Bấm vào đèn lồng -> mở lời chúc + ảnh
    const lanternHit = hitMesh;
    selectedLantern = lanternHit.userData.parentLantern || lanternHit.parent;
    ...
    wishTitle.textContent = "Thông Điệp Gửi Cậu";
    wishTitle.style.color = "#ffd700";
    wishModal.querySelector(".tag").textContent = "✦ Trung Thu An Lành ✦";
    wishModal.querySelector(".tag").style.color = "#ff9ebb";

    const card = wishModal.querySelector(".wish-card");
    card.style.borderColor = "rgba(255, 215, 0, 0.5)";
    card.style.boxShadow = "0 0 35px rgba(255, 182, 193, 0.3), inset 0 0 15px rgba(255, 215, 0, 0.2)";

    wishText.textContent = `"${selectedLantern.userData.wish}"`;
    wishImage.style.display = "block";
    wishImage.parentElement.style.display = "flex";

    // Mỗi ảnh có thể có tỉ lệ khác nhau.
    // Đợi ảnh load xong để trình duyệt tự tính khung theo đúng tỉ lệ ảnh.
    wishImage.onload = () => {
      const container = wishImage.parentElement;
      const maxHeight = Math.min(window.innerHeight * 0.45, 420);
      const naturalRatio = wishImage.naturalWidth / wishImage.naturalHeight;

      if (naturalRatio > 0) {
        const availableWidth = container.clientWidth;
        const calculatedHeight = availableWidth / naturalRatio;

        container.style.height = Math.min(calculatedHeight, maxHeight) + "px";
        wishImage.style.height =
          Math.min(calculatedHeight, maxHeight) + "px";
      }
    };
...
function closeWishCard(e) {
  ...
  wishImage.parentElement.style.height = "auto";
  wishImage.style.display = "block";
  wishImage.parentElement.style.display = "flex";
  wishTitle.textContent = "Thông Điệp Gửi Cậu";
  wishTitle.style.color = "#ffd700";
